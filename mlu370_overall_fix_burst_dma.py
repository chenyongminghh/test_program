...
def mt_run_DMA_IPU_Test(dvt_dir, ipu_cmd, ipu_dvt_name, ipu_loops, dma_dvt_name, dma_loops):
    ...
    for mcPort in range(mcPorts):
        time.sleep(0.1)
        p1 = mp.Process(target=CAM.mlu370_new_power_one_card, args=(q1, mcPort, 'mlu370 DMA PITest-Fix-Burst', ipu_cmd, high_loops, low_loops, inner_loops, dvt_dir, ipu_dvt_name, 27))
        p1.start()
        p_lst.append(p1)
        
        time.sleep(0.1)
        p2 = mp.Process(target=CAM.mlu370_dvt_one_card, args=(q2, mcPort, 'mlu370 Fix-Burst PCIE-DMA', 'driver', dma_dvt_name, ['mlu370_pcie_PCIE_DRV_01'], dma_loops, dvt_dir, 27))
        p2.start()
        p_lst.append(p2)
    time.sleep(0.5)
...
def main(args):
    case_name = 'mlu370 PITest Fix Burst and DMA Test'
