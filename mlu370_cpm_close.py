...
def cpm_close_one_card(mcPort, mcSN, caseName, debug_tool):
    mcPort = str(mcPort)
    log_fname = osp.join(logs_path, '{}/{}'.format(CAM.format_dirname(mcPort, mcSN, 'voltage'), CAM.format_fname(mcPort, mcSN, 'cpm_close')))
    logging.info('{} Start  on Card{} <{}>'.format(caseName, mcPort, mcSN))
    
    test_cmd = [debug_tool, '-i', mcPort, '-f', '16']
    ...
    if "ERROR" in return_context or "cpm close check ok" not in return_context:
...
def main(args):
    ...
    for mcPort in range(mcPorts):
        ...
        if not cpm_close_one_card(mcPort, mcSN, case_name, debug_tool_path):
            test_result = False
