...
def mt_run_DDR_Test(dvt_dir, ddr_tool, ddr_loops):
    ...
    for mcPort in range(mcPorts):
        time.sleep(0.1)
        p = mp.Process(target=CAM.mlu370_ddr_power_one_card, args=(q, mcPort, 'mlu370 DDR Maxpower Start', ddr_tool, ddr_loops, dvt_dir, 27, '19'))
        p.start()
        p_lst.append(p)
    time.sleep(0.5)
...
def main(args):
    case_name = 'mlu370 DDR Maxpower Polling Test'
