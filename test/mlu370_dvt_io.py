...
def main(args):
    case_name = 'mlu370 DVT IO Test'
    logging.info(case_name, section=True)
    
    test_list = ['mlu370_io_IO_INST_ALL_READ_U1', 'mlu370_io_IO_INST_ALL_WRITE_U1']
    logging.info('Running <{}> IO test cases'.format(len(test_list)))
    
    test_loops = args.loops.strip() if args.loops else '1'
    logging.info('Running DVT IO Test <{}> loops'.format(test_loops))
