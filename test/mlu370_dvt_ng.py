...
def main(args):
    case_name = 'mlu370 DVT NG Test'
    logging.info(case_name, section=True)
    
    test_list = CAM.get_dvt_list('ng')
    logging.info('Running <{}> NG test cases'.format(len(test_list)))
    
    test_loops = args.loops.strip() if args.loops else '1'
    logging.info('Running DVT NG Test <{}> loops'.format(test_loops))
