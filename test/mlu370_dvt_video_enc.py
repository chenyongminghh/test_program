...
def main(args):
    case_name = 'mlu370 DVT Video Enc Test'
    logging.info(case_name, section=True)
    
    test_list = CAM.get_dvt_list('video_enc')
    logging.info('Running <{}> Video Enc test cases'.format(len(test_list)))
    
    test_loops = args.loops.strip() if args.loops else '1'
    logging.info('Running DVT Video Enc Test <{}> loops'.format(test_loops))
