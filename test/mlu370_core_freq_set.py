...
def ipu_frequency_set(mcPort, mcSN, caseName, debug_tool):
    mcPort = str(mcPort)
    log_fname = osp.join(logs_path, '{}/{}'.format(CAM.format_dirname(mcPort, mcSN, 'voltage'), CAM.format_fname(mcPort, mcSN, 'ipu_frequency_set')))
    logging.info('{} Start  on Card{} <{}>'.format(caseName, mcPort, mcSN))
    
    test_cmd = [debug_tool, '-i', mcPort, '-w', '-a', '0x00368070', '-v', '0x84b0']
...
def main(args):
    case_name = 'mlu370 IPU Frequency Set'
    ...
    for mcPort in range(mcPorts):
        ...
        if not ipu_frequency_set(mcPort, mcSN, case_name, debug_tool_path):
            test_result = False
