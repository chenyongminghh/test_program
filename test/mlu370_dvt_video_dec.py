...
def main(args):
    case_name = 'mlu370 DVT Video Dec Test'
    logging.info(case_name, section=True)
    
    test_list = CAM.get_dvt_list('video_dec')
    logging.info('Running <{}> Video Dec test cases'.format(len(test_list)))
    
    test_loops = args.loops.strip() if args.loops else '1'
    logging.info('Running DVT Video Dec Test <{}> loops'.format(test_loops))
