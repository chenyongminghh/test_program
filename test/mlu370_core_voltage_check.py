...
def mt_run_test(caseName, debug_tool, core_voltage):
    ...
    for mcPort in range(mcPorts):
        ...
        if not CAM.check_core_voltage(mcPort, mcSN, debug_tool, core_voltage, display=True):
            allPassed = False
            failure_message = '{} Failed on Card{}'.format(caseName, mcPort)
            CAM.record_card_fail(mcPort, mcSN, 'mlu370 Check Core Voltage', failure_message)
    return allPassed

def main(args):
    core_voltage = args.voltage.strip() if args.voltage else '70'
    case_name = 'Check Core Voltage to 0.{}V'.format(core_voltage)
