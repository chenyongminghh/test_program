...
def main(args):
    case_name = 'mlu370 PITest Fix Maxpower U8 IO Test'
    logging.info(case_name, section=True)
    
    board_type = CAM.get_mlu370_type('0')
    if board_type not in ['X4', 'X4K', 'X4L', 'EVBD']:
        logging.info('Running mlu370-{} type boards, skip'.format(board_type))
        return
    ...
    dvt_dir = 'dvt-power'
    dvt_name = 'fix_maxpower_u8_io'
    ipu_cmd = 'mlu370_ipu_IPU_INST_FIX_MAXPOWER_U8_IO'
    CAM.mlu370_dimz('fix_test', 'maxp_u8_io', dimz_loops)
