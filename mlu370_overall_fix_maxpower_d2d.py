...
def mt_run_DMA_IPU_Test(dvt_dir, ipu_cmd, ipu_dvt_name, ipu_loops, gdmac_cmd, d2d_dvt_name, d2d_loops):
    ...
    for mcPort in range(mcPorts):
        time.sleep(0.1)
        p1 = mp.Process(target=CAM.mlu370_new_power_one_card, args=(q1, mcPort, 'mlu370 D2D PITest-Fix-Maxpower', ipu_cmd, high_loops, low_loops, inner_loops, dvt_dir, ipu_dvt_name, 30))
        p1.start()
        p_lst.append(p1)
        
        time.sleep(0.1)
        p2 = mp.Process(target=CAM.mlu370_new_gdmac_test, args=(q2, mcPort, 'mlu370 Fix-Maxpower D2D-GDMAC', gdmac_cmd, d2d_loops, dvt_dir, d2d_dvt_name, 30))
        p2.start()
        p_lst.append(p2)
    time.sleep(0.5)
...
def main(args):
    case_name = 'mlu370 PITest Fix Maxpower and D2D Test'
