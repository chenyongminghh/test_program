...
def cpm_enable_one_card(mcPort, mcSN, caseName, debug_tool, core_voltage):
    mcPort = str(mcPort)
    ...
    # ./MLU370_Debug_Tool -i 0 -f 13 -y 20 -z 40 -s core_voltage
    board_type = CAM.get_mlu370_type(mcPort)
    if board_type in ['X8', 'D2', 'M8']:
        test_cmd = [debug_tool, '-i', mcPort, '-f', '13', '-y', '20', '-z', '40', '-s', core_voltage]
    elif board_type in ['S4', 'S8', 'X4', 'X4L', 'X4K', 'X9', 'X9L']:
        test_cmd = [debug_tool, '-i', mcPort, '-f', '13', '-y', '10', '-z', '30', '-s', core_voltage]
    else:
        logging.error('{} Failed on Card{} with un-correct board type'.format(caseName, mcPort))
        return False
    ...
    if "ERROR" in return_context or "complete cpm step 1" not in return_context:
...
def main(args):
    ...
    core_voltage = core_voltage + '0'
    logging.info('Core Voltage is {}mv'.format(core_voltage))
    ...
    for mcPort in range(mcPorts):
        ...
        if not cpm_enable_one_card(mcPort, mcSN, case_name, debug_tool_path, core_voltage):
            test_result = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--voltage', action='store', choices=['70', '85', '86'], help='Board Core Voltage')
